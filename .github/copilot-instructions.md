...
This is the **ioBroker SolisCloud adapter** (`iobroker.soliscloud`), which reads solar inverter data from the Solis/Ginlong SolisCloud API and stores it in ioBroker states. Users must request API access from Ginlong and configure their API key, secret, plant ID, and API URL.

### Adapter-Specific Context
- **Adapter Name:** soliscloud
- **Primary Function:** Polls the SolisCloud REST API to retrieve solar energy production data (station details, inverter details, inverter list, EPM details) and expose them as ioBroker states.
- **Key Dependencies:** SolisCloud REST API (cloud-based, requires user credentials); uses HMAC-SHA1 signed requests with Content-MD5 headers. The `axios` library is used for HTTP requests (production dependency).
- **Configuration Requirements:** `apiKey` (encrypted), `apiSecret` (encrypted), `apiUrl`, `plantId`, `pollInterval` (45–1800 seconds), `debugLogging` boolean, and optional EPM API settings. Configured via `admin/jsonConfig.json`.
- **API Authentication:** Requests are signed using HMAC-SHA1 with a canonical string composed of `Content-MD5 + Content-Type + Date + Resource`. The API ID and Secret are used to construct the `Authorization` header.
- **Data Flow:** `onReady()` → `createObjects()` → polling via `setInterval` → `getStationDetails()` / `getInverterDetails()` / `getInverterList()` / `getEpmDetails()` in `src/lib/apiHelper.ts`.
...
#### SolisCloud-Specific API Testing

The SolisCloud API does not provide public demo credentials. Integration tests requiring live API access should:
- Use environment secrets (`SOLIS_API_KEY`, `SOLIS_API_SECRET`, `SOLIS_PLANT_ID`, `SOLIS_API_URL`) stored in GitHub repository secrets
- Be run only in CI when secrets are available (use `if: env.SOLIS_API_KEY != ''`)
- Validate HMAC-SHA1 signature generation logic via unit tests with known request/response pairs
- Mock HTTP responses using pre-recorded API response fixtures for offline testing
...
- **Current:** The adapter uses `axios` (production dependency) for all HTTP requests to the SolisCloud API
- **Future:** Consider migrating to native `fetch` API (Node.js 20+ is already required) to reduce dependencies

**Existing axios pattern in this adapter:**
```typescript
...
- **Scheduling:** Use adapter built-in timers and intervals (`this.setInterval`)
...
## SolisCloud-Specific Patterns

### API Signature Generation

SolisCloud uses HMAC-SHA1 signed requests. When adding new API calls, follow this signing pattern from `src/lib/apiHelper.ts`:

```typescript
// Canonical string: Content-MD5 + \n + Content-Type + \n + Date + \n + Resource
const contentMd5 = crypto.createHash('md5').update(body).digest('base64');
const date = new Date().toUTCString();
const stringToSign = `POST\n${contentMd5}\napplication/json\n${date}\n${resource}`;
const hmac = crypto.createHmac('sha1', apiSecret).update(stringToSign).digest('base64');
const authorization = `API ${apiKey}:${hmac}`;
```

### State Object Naming

ioBroker state IDs are derived from `plantId` via `this.name2id()` which replaces forbidden characters with underscores. Always sanitize user-provided IDs before using them in state paths.

### Adding New Data Points

When adding new values from the SolisCloud API:
1. Add the state definition in `src/lib/createObjects.ts`
2. Parse and set the value in the appropriate `get*Details()` function in `src/lib/apiHelper.ts`
3. Use `adapter.setStateChangedAsync()` to avoid unnecessary state updates
4. Document the new state in README with its unit and description
